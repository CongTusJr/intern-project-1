...
const BookNow:React.FC = () => {
  ...
  return (
    <div className='block text-center relative mt-[-88px] bg-textA z-10'> 
        <div className='grid grid-cols-1 md:grid-cols-6  align-middle w-full'>
            ...
            <div className=' text-white bg-black left py-[17px] px-[15px]'>
                ...
                <div className='flex justify-center items-center absolute top-[17%] bg-white w-[40px] h-[40px] rounded-full left-[233px] mt-2 z-20'>
                    <FaAngleRight className='text-[#ae8545] h-4 '/>
                </div>
            </div>

            <div className='relative px-[15px] left-5'>
                <div className='mt-[25px]'>
                    <select name="DateIn"  className='py-[6px] px-3 w-full text-[#333] border border-solid border-[#333] h-10'>
                        {optionPeoples.map((optionPeople)=>(
                            <option value={optionPeople.value}  key={optionPeople.key}>{optionPeople.lable}</option>
                        ))}
                    </select>
                </div>
            </div>
{/* Check in */}
<div className='relative px-[15px] left-[10px]'>
      ...
      <div className='relative px-[15px]'>
        <div className="mt-[25px] relative">
          ...
          {showCheckoutCalendar && (
            <div>
              <CalendarComponent onSelect={handleCheckoutSelect} disabledDate={disabledDateCheckout} />
            </div>
          )}
        </div>
      </div>

            <div className='relative px-[15px] right-[10px]'>
                <div className='mt-[25px]'>
                    <select name="DateOut" className='py-[6px] px-3 w-full text-[#333] border border-solid border-[#333] h-10'>
                        {optionChilds.map((optionChild)=>(
                            <option value={optionChild.value}  key={optionChild.key}>{optionChild.lable}</option>
                        ))}
                    </select>
                </div>
            </div>
            ...
            <div className='relative px-[15px]'>
                ...
                <button type='submit' className='overflow-visible transform-none cursor-pointer relative block w-full h-full text-center py-[28.5px] bg-[#755727] border border-solid border-[#755727]'>
