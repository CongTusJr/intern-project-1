          ...
          <Row className='gap-5 leading-[25px]'>
            <Col span={8} >
              ...
              <div className='text-[34px] h-[56px] flex py-[14px] ml-[-12px]'>
                {ListIcons.map((icons)=>(
                  <a key={icons.key} href={icons.url} className='flex border-[1px] border-white hover:border-goldlg h-[32px] w-[34px] justify-center mx-[15px] p-[5px] items-center'>
            ...
            <Col span={6} className='inline-block font-myriad text-white'>
              ...
              {menuFt.map((menu)=>(
                <p className='mb-[10px] cursor-pointer' key={menu.key}>{menu.name}</p>
              ))}
            </Col>
            <Col span={8}>
